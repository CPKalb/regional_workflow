...
#These colors are single color colortable RGB values from colorbrewer 2.0 (http://colorbrewer2.org/)
   ...
   purple0 = (255/255., 255/255., 255/255.)
   ...
   purple5p5 = (142/255., 139/255., 192/255.)
   purple6 = (128/255., 125/255., 186/255.)
   purple7 = (106/255., 81/255., 163/255.)
   purple8 = (84/255., 39/255., 143/255.)
   purple9 = (63/255., 0/255., 125/255.)
   purple10 = (46/255., 0/255., 104/255.)
   purple11 = (32/255., 0/255., 87/255.)
   purple12 = (24/255., 0/255., 68/255.)
...
#######################
   ...
   fig = P.figure(figsize=(6.5,9))

   ax1 = fig.add_axes([0.01,0.15,0.98,0.80])
...
def paintqc_plot(map, fig, ax1, ax2, ax3, x, y, vx, vy, plot, var1, var2, radmask, ob_count, member_count, t, init_label, valid_label, domain, outdir, t_star, t_plus, method, dz_thresh):
...
##   p3 = map.contourf(vx, vy, radmask, colors=[cb_colors.gray5, cb_colors.gray5], levels=[0.1, 1000.], alpha=0.6)
      ...
      for n in range(0, var2.shape[0]):
        ...
        print("levels:")
        print(plot.var2_levels)

        var2[n,:,:] = gaussian_filter(var2[n,:,:], .1)   
    
        purples = [ cb_colors.purple2, cb_colors.purple3, cb_colors.purple4, cb_colors.purple5, cb_colors.purple5p5, cb_colors.purple6, cb_colors.purple7, cb_colors.purple8, cb_colors.purple9, cb_colors.purple10, cb_colors.purple11, cb_colors.purple12]
        #cmap_dz = matplotlib.cm.get_cmap('Purples', len(plot.var2_levels)+3)
        cmap_dz = matplotlib.cm.get_cmap('RdYlBu_r', len(plot.var2_levels+2))
        #cmap_dz = ctables.registry.get_colortable('NWSReflectivity')
        
        cmap_dz_list = [ cmap_dz(i) for i in range(2,len(plot.var2_levels))]
        cmap_dz_list = [[1,1,1,1]] + cmap_dz_list
        print('var2_levels: ' + str(len(plot.var2_levels)))
        print('cmap.n: ' + str(cmap_dz.N))
        norm = matplotlib.colors.BoundaryNorm(plot.var2_levels, cmap_dz.N)

        p2 = map.contourf(x, y, var2[n,:,:], colors=cmap_dz_list, norm=norm, levels=plot.var2_levels, alpha=1.0, extend=plot.extend)
#        p22 = map.contour(x, y, var2[n,:,:], colors=[plot.var2_colors[n]], levels=plot.var2_levels, alpha=.9, extend=plot.extend, linewidths=.3)
#        p22 = map.contour(x, y, var2[n,:,:], colors=[cb_colors.purple1, cb_colors.purple2, cb_colors.purple3, cb_colors.purple4], levels=plot.var2_levels, alpha=.9, extend=plot.extend, linewidths=.3)
        #proxy += [matplotlib.pyplot.Rectangle((0,0),1,1,color = cb_colors.purple7)]
          # for pc in p2.collections
        bounds = plot.var2_levels
        matplotlib.pyplot.colorbar(cmap=cmap_dz_list, norm=norm, fraction=0.046, pad=0.04, extend='neither', orientation='horizontal', shrink=1.0, aspect=40)
        
      #sigma = 2.25 # how much do we want to smooth for observation contours
...
#      p12 = map.contour(vx, vy, var1, colors=plot.var1_colors, levels=[.1, .3, .5, .7, .9], alpha=1, extend=plot.extend, linewidths=[1.0, 1.0, 1.0, 1.4, 1.5])
      p12 = map.contour(vx, vy, var1, colors=plot.var1_colors, levels=[.6], alpha=1, extend=plot.extend, linewidths=[1.75])
      ...
      for l, s in zip(p12.levels, strs):
        fmt[l] = s
      matplotlib.pyplot.clabel(p12, p12.levels, inline=True, fmt=fmt, fontsize=8, inline_spacing=0)
#      matplotlib.pyplot.title('REFC_'+method+'_'+str(dz_thresh))
      #proxy += [matplotlib.pyplot.Rectangle((0,0),1,.5,color = plot.var1_colors[0])]
      #matplotlib.pyplot.legend(proxy, ['REFC_'+method+'_'+str(dz_thresh), 'MRMS'], loc ="upper left")
      ###LEGEND###
...
########### Save figure as .png, then clear all plots from basemap template: ################
   ...
   fig_name = outdir + plot.name + '_' + outtime + '.png'
   P.savefig(fig_name, format="png", bbox_inches='tight', dpi=300)
